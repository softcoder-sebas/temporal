...
namespace MyMarket_ERP
{
    public partial class Inventario : Form
    {
        ...
        private void Inventario_Shown(object sender, EventArgs e)
        {
            if (_split == null)
            {
                // The desired split host is the panel that contains the main body layout (which itself contains
                // the content card and the ranking card). In the designer the structure is:
                // container (Panel) -> bodyLayout (TableLayoutPanel) -> contentCard (Panel) -> grid
                // We try to locate that container and build the split at that level so the detail panel
                // appears to the right of BOTH the content card and the ranking card.
                Panel? host = null;
                var gridParent = grid.Parent;
                // gridParent is contentCard; its parent should be the TableLayoutPanel (bodyLayout)
                if (gridParent?.Parent is TableLayoutPanel bodyLayout && bodyLayout.Parent is Panel container)
                {
                    host = container;
                }
                else if (gridParent is Panel p)
                {
                    host = p;
                }

                if (host != null)
                    BuildSplitOrdenes(host);
            }

            // Ensure the evaluate button is placed next to the toggle and kept aligned
            PositionEvaluateButton();
            if (btnToggleDetalle?.Parent != null)
            {
                btnToggleDetalle.Parent.SizeChanged += (_, __) => PositionEvaluateButton();
                btnToggleDetalle.SizeChanged += (_, __) => PositionEvaluateButton();
                btnEvaluarProveedor.SizeChanged += (_, __) => PositionEvaluateButton();
            }
        }
        ...
        private void BuildSplitOrdenes(Panel host)
        {
            ...
            // Add split to the host and move the left content (either the content card or the
            // whole body layout that contains both content card and ranking card) into Panel1.
            host.Controls.Add(_split);
            _split.BringToFront();

            // Determine the left control to move. Prefer to move the parent TableLayoutPanel that
            // contains the content and ranking cards (so both stay left of the detail panel).
            Control? leftControl = null;
            if (grid.Parent?.Parent is TableLayoutPanel bodyLayout)
            {
                leftControl = bodyLayout;
            }
            else if (grid.Parent is Control c)
            {
                leftControl = c;
            }
            ...
            if (leftControl != null)
            {
                leftControl.Parent = _split.Panel1;
                leftControl.Dock = DockStyle.Fill;
            }

            // Move the existing pnlDetalle into the right panel of the split
        ...
        private void PositionEvaluateButton()
        {
            if (btnEvaluarProveedor == null || btnToggleDetalle == null) return;

            var parent = btnToggleDetalle.Parent ?? btnEvaluarProveedor.Parent;
            if (parent == null) return;

            // Ensure evaluate button is a child of the same pager panel
            if (btnEvaluarProveedor.Parent != parent)
            {
                parent.Controls.Add(btnEvaluarProveedor);
            }

            // Place it right after the toggle button with a small gap
            btnEvaluarProveedor.Top = btnToggleDetalle.Top;
            btnEvaluarProveedor.Left = btnToggleDetalle.Left + btnToggleDetalle.Width + 8;
            btnEvaluarProveedor.BringToFront();
        }
    }
